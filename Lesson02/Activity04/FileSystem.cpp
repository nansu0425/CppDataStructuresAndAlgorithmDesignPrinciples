...
void FileSystem::printListPath(const std::string& path)
{
	if (path.empty())
	{
		return;
...
void FileSystem::printListDir(Node* ptrDir)
{
	for (Node* ptrChild : ptrDir->ptrsChild)
	{
		const char prefix = (ptrChild->isDir) ? 'd' : '-';
		std::cout << '\t' << prefix << " " << ptrChild->name << std::endl;
	}
}
